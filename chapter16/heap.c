...
pthread_barrier_t done_barrirer;
...
void *printer_thread_body(void *arg)
{
    pthread_barrier_wait(&fill_barrier);
    printf(">> %s\n", shared_array);
    pthread_barrier_wait(&done_barrirer);
    return NULL;
}

void *delloc_thread_body(void *arg)
{
    pthread_barrier_wait(&done_barrirer);
    free(shared_array);
    pthread_barrier_destroy(&alloc_barrier);
    pthread_barrier_destroy(&fill_barrier);
    pthread_barrier_destroy(&done_barrirer);
    return NULL;
}
...
int main(int argc, char const *argv[])
{
    ...
    pthread_barrier_init(&done_barrirer, NULL, 2);
    ...
    res = pthread_create(&dealloc_thread, &attr, delloc_thread_body, NULL);
    CHECK_RESULT(res);
