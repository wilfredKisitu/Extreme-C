...
int main(int argc, char const *argv[])
{
    if(argc < 4) {
        ...
        printf("[number-rows] [number-columns]\n");
        exit(1);
    }

    const char* operations = argv[1];
    ...
    if(strcmp(operations, "print") == 0) {
        print_matrix(matrix, rows, columns);
        print_flat(matrix, rows, columns);
    } else if(strcmp(operations, "friendly-sum") == 0){
        int sum = friendly_sum(matrix, rows, columns);
        printf("Friendly sum: %d\n", sum);
    } else if(strcmp(operations, "not-friendly-sum") == 0){
